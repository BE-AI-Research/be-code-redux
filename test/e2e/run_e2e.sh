...
# a small context_tokens budget force maybeCompact to collapse the two older
# reads to stubs before the fourth call (compact_with_model:false keeps the
# harness on that cheap collapse path instead of falling through to a
# model-written summary, which replaces every tool result — the redundant
# read's own footer included — with a generic stub; collapse-only leaves the
# newest tool result, footer and all, untouched). The mock reports whether
# the Working memory block survived that and whether the redundant read
# still carried its "already read" footer.
{
	printf 'package main\n\nfunc main() {}\n'
	i=1
	while [ "$i" -le 297 ]; do
