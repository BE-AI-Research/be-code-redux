...
**Amended after the run-time review (2026-09-20).** The decision above to *not ship* these assemblies was half wrong. Two things were missing. `UseCodebase` registers a code base for the package assembly only, so nothing put `BECode.Bridge.dll` — let alone its dependencies — on `devenv.exe`'s probing path: the package now carries `[ProvideBindingPath]`. And betting that Visual Studio carries *every* assembly the bridge needs (`System.Threading.Channels` and `System.Text.Encodings.Web` are the doubtful ones) was a bet nobody could check. Because the versions are held at 6.0.x — older than anything Visual Studio 17.6+ redirects to — shipping them is safe in both directions: where `devenv.exe.config` redirects an assembly, Visual Studio's own newer copy still wins and ours is ignored; where it carries none, ours is found on the binding path instead of the load failing. So the VSIX **ships** the 6.0.x BCL set, and still never ships Roslyn or the Visual Studio SDK. Holding at 6.0.x remains the point of this section; `build.ps1` lists the container's contents and fails if `BECode.Bridge.dll` is absent.
...
## 7. What the run-time review changed

One review by a reader who knows the SDK's run-time behaviour, after the code compiled. It confirmed the two outcomes that matter most are not reachable — nothing here can take down Visual Studio or fail package load — and changed these decisions:

- **§1.1:** ship the 6.0.x BCL set and add `[ProvideBindingPath]` (amended in place above).
- **§2.2:** the lock file is **republished** whenever the folder list changes, not written once. The package loads with no solution open in the ordinary case; a lock advertising no folders is one the harness never attaches to. Open Folder mode is also read at initialise (`VSPROPID_IsInOpenFolderMode`), not only from its event.
- **§2.3:** every `TaskCompletionSource` is created with `RunContinuationsAsynchronously` — otherwise a click or a debugger event runs the rest of the tool call, the bridge's JSON serialisation and the socket write inline on the UI thread, which no analyzer sees. `Guard` leaves the UI thread before it returns or rethrows. Every callback Visual Studio invokes is fenced.
- **§4 `ConfigsAsync`:** the startability heuristic is gone. Every loaded project that is not a solution folder is listed, through the same `IVsSolution` enumeration as the workspace folders (so projects inside solution folders are seen), startup project first. Over-listing is harmless — `debug_start` on a class library fails with Visual Studio's own message; under-listing hides a project the model then never tries.
- **§4 `StartAsync`:** "build failed" is decided only once `SolutionBuild.BuildState` says the build has finished, never at a fixed delay; `LastBuildInfo` describes the *previous* build until then. One semaphore serialises start, continue, step and stop.
- **§3.4:** an `ErrorItem.FileName` that is not rooted (common for build errors) is resolved against its project's directory before it is compared or returned; the read is capped at 5 000 items and says so when it truncates.
- **§3.5:** the information bar names the file always, and is closed in the `finally` — on the main-window fallback a bar that is merely unadvised stays pinned to Visual Studio.
- **Logging:** the Activity Log may not be writable from a background thread, and it is the owner's only diagnostic surface, so every entry is mirrored to `<home>\.be-code\visualstudio.log`.
