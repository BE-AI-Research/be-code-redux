...
Decided by the owner: the repository had no LICENSE file, though `vscode/package.json` declared MIT — it now carries the MIT licence, the same text and copyright holder as `vscode/LICENSE`. Still the owner's to decide: the Visual Studio extension's own version is `0.1.0` (it is what the lock file and `serverInfo` report) while the release is 0.12.0.
