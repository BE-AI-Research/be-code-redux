...
7. Ask for a small file edit → expect an editor diff and the Accept/Reject dialog; Reject once, then Accept. (That is the editor *alone*. For the shared review — the same change offered in the editor and in every attached terminal at once, first answer winning — see item 13 of `docs/live-checklist.md`.)
