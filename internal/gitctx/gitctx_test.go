...
func TestSnapshotInEmptyRepoAndOutsideRepo(t *testing.T) {
	...
	ctx := context.Background()
	empty := t.TempDir()
	if out, err := git(ctx, empty, "init", "-q", "-b", "main"); err != nil {
		t.Fatalf("git init: %v %s", err, out)
	}
	os.WriteFile(filepath.Join(empty, "f.txt"), []byte("x\n"), 0o644)
...
// TestCommitMessageIsNeverShellInterpreted: the commit message is written by
// the model, so it must reach git as one argv element. Composing a shell
// line ("git commit -m ...") let $(...) inside a message execute.
func TestCommitMessageIsNeverShellInterpreted(t *testing.T) {
	dir := gitRepo(t)
	ctx := context.Background()
	os.WriteFile(filepath.Join(dir, "b.txt"), []byte("x"), 0o644)

	const msg = `fix bug $(touch PWNED) 'quoted' "dq"`
	if _, err := Commit(ctx, dir, msg); err != nil {
		t.Fatalf("Commit: %v", err)
	}
	subject, err := git(ctx, dir, "log", "-1", "--format=%s")
	if err != nil {
		t.Fatal(err)
	}
	if subject != msg {
		t.Errorf("subject = %q, want %q", subject, msg)
	}
	if _, err := os.Stat(filepath.Join(dir, "PWNED")); err == nil {
		t.Fatal("the message's $(touch PWNED) executed: gitctx composed a shell line")
	}
}
